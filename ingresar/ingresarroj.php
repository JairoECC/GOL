...
$tar_roj = $_POST['txtroja'];
$equipo = $_POST['txtequipo'];

if (empty($nombre) || empty($tar_roj) || empty($equipo)) {
...
if ($_FILES["foto_perfil"]["size"] > 0) {
    ...
    // Si todo está bien, intenta subir el archivo
        ...
        if (move_uploaded_file($_FILES["foto_perfil"]["tmp_name"], $target_file)) {
            ...
            $sentencia = $bd->prepare("INSERT INTO jugador4 (nombre, equipo, tar_roj, usu_id, foto) VALUES (?, ?, ?, ?, ?);");
            $resultado = $sentencia->execute([$nombre, $equipo, $tar_roj, $usu_id, $target_file]);
    ...
    $sentencia = $bd->prepare("INSERT INTO jugador4 (nombre, equipo, tar_roj, usu_id) VALUES (?, ?, ?, ?);");
    $resultado = $sentencia->execute([$nombre, $equipo, $tar_roj, $usu_id]);
