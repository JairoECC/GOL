...
if (empty($nombre) || empty($asistencia) || empty($equipo)) {
...
if ($_FILES["foto_perfil"]["size"] > 0) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["foto_perfil"]["tmp_name"], $target_file)) {
            ...
            $sentencia = $bd->prepare("UPDATE jugador2 SET nombre = ?, equipo = ?, asistencia = ?, foto = ? WHERE nombre = ?;");
            $resultado = $sentencia->execute([$nombre, $equipo, $asistencia, $target_file, $nombreExistente]);
    ...
    $sentencia = $bd->prepare("UPDATE jugador2 SET nombre = ?, equipo = ?, asistencia = ? WHERE nombre = ?;");
    $resultado = $sentencia->execute([$nombre, $equipo, $asistencia, $nombreExistente]);
