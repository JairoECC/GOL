...
if ($_FILES["foto_perfil"]["size"] > 0) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["foto_perfil"]["tmp_name"], $target_file)) {
            ...
            $sentencia = $bd->prepare("UPDATE jugador SET nombre = ?, equipo = ?, goles = ?, foto = ? WHERE nombre = ?;");
            $resultado = $sentencia->execute([$nombre, $equipo, $goles, $target_file, $nombreExistente]);
    ...
    $sentencia = $bd->prepare("UPDATE jugador SET nombre = ?, equipo = ?, goles = ? WHERE nombre = ?;");
    $resultado = $sentencia->execute([$nombre, $equipo, $goles, $nombreExistente]);
