...
if (!isset($_SESSION['nombre'])) {
    ...
    if (isset($_POST['nombre'])) {
        $nombre = $_POST['nombre'];
        $query = "SELECT foto, nombre, equipo, tar_roj FROM jugador4 WHERE usu_id = :usu_id AND nombre LIKE :nombre ORDER BY tar_roj DESC";
        $stmt = $bd->prepare($query);
        $stmt->execute(['usu_id' => $usu_id, ':nombre' => "%$nombre%"]);
        $jugador = $stmt->fetchAll(PDO::FETCH_OBJ);
    } else {
        $query = "SELECT foto, nombre, equipo, tar_roj FROM jugador4 WHERE usu_id = :usu_id ORDER BY tar_roj DESC";
        $stmt = $bd->prepare($query);
        ...
        <h3>TARJETAS ROJAS</h3>
            <table class="tabla-goleador">
                ...
                <tr>
                    ...
                    <td id="td-goals"><?php echo $dato->tar_roj; ?></td>
                    <td id="td-edit">
